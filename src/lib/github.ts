...
/**
 * GitHub file object from API response
 */
    ...
    const fetchMethod2 = async (): Promise<string> => {
      try {
        ...
        if (typeof response.data === "string") {
          ...
          response.data &&
          (response.data as { files: GitHubFile[] }).files
        ) {
          return (response.data as { files: GitHubFile[] }).files
            .map((file: GitHubFile) => {
              const fileHeader =
                file.status === "renamed"
        ...
        if (data && (data as { files: GitHubFile[] }).files) {
          return (data as { files: GitHubFile[] }).files
            .map((file: GitHubFile) => {
              const fileHeader =
                file.status === "renamed"
                  ? `diff --git a/${file.previous_filename || "unknown"} b/${file.filename}\n`
